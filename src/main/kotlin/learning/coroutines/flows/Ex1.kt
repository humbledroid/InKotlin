...
fun placeOrder(vararg mealCombo: MealCombo) {
    val items = getItemsForMealCombo(mealCombo.toList())

    println("Fulfilling an order that has ${mealCombo.size} combos in it.")
    println("    Combos in this order: ${mealCombo.joinToString { it.description }}")
...
fun getItemsForMealCombo(combos: List<MealCombo>): List<String> {
    return combos
        .flatMap { it.items }
        .map {
