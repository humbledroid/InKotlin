...
fun main() {
    runBlocking {
        launch(Dispatchers.Default + CoroutineName("Cole")) { grillChicken() }
        launch(Dispatchers.Default + CoroutineName("Jane")) { steamVegetables() }
        launch(Dispatchers.Default + CoroutineName("Will")) { bakePotatoes() }
    }
}
...
suspend fun actively(string: String, duration: Duration) {
    kotlin.io.println("${coroutineContext.name} >> $string")
