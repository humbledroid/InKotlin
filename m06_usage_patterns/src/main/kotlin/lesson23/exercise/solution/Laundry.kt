package lesson23.exercise.solution
        ...
        val result = customerOrders.map { order ->
            async {
                order to order.garments
                    .map { garment -> async { process(garment) } }
                    .awaitAll()
            }
        }.awaitAll()
...
suspend fun process(garment: Garment): Garment =
    garment
        .also { println("${it.label} - Received 🆕") }
        .receive()
        .soak()
        .wash()
        .dry()
        .iron()
        .bagAndBill()
        .also { println("${it.label} - Completed ✅") }
