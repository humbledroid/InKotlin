package lesson25.content.state01
...
fun main() {
    runBlocking(Dispatchers.Default) {
        val job1 = async { WorkOrder("🚙", Symptom.SQUEAKING_SOUND).diagnose().repair().cost }
        val job2 = async { WorkOrder("🚗", Symptom.WILL_NOT_START).diagnose().repair().cost }
        val job3 = async { WorkOrder("🚚", Symptom.BURNING_SMELL).diagnose().repair().cost }
