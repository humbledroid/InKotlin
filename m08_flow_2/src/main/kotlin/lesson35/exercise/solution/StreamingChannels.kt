package lesson35.exercise.solution
...
suspend fun main() {
    ...
    allChannels.flattenMerge(3).collect { watch(it) }

    println("Binging Benny ---------------------------------")
    allChannels.flattenConcat().collect { watch(it) }

    println("Indecisive Inigo ------------------------------")
    allChannels.flatMapLatest { it }.collect { watch(it) }
}
