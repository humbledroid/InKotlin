package lesson34.exercise.solution
...
suspend fun main() {
    println("MUSIC -----------------------------------------")
    merge(guitar, drums).collect(::println)

    println("ATTENDANCE ------------------------------------")
    wristbands.zip(raffleTickets, ::Attendee).collect(::println)

    println("LIGHTING --------------------------------------")
    combine(colorControls, brightnessControls, ::LightingSnapshot).collect(::println)
}
