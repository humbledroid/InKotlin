package lesson33.exercise.solution
...
fun main() {
    ...
    runBlocking(Dispatchers.Default) {
        launch {
            deliveries
                .onEach { logInbound("$it Received item") }
                .buffer()
                .collect { Inventory.add(it) }
        }
        launch {
            purchases
                .buffer()
                .onEach { logOutbound("$it Customer purchasing") }
                .collect { Inventory.purchase(it) }
        }
    }
