package lesson31.exercise.solution
...
fun main() {
    runBlocking {
        exhibits.split(" ").forEach { exhibit ->
            flow { emit(CellService.sendTextMessage(exhibit)) }
                .catch { throwable -> println(throwable.message); throw throwable }
                .retry { throwable -> throwable is ServiceInterruptionException }
                .collect { println(it.message) }
        }
    }
}
