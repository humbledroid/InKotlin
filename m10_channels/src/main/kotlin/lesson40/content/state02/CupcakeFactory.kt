package lesson40.content.state02
...
fun main() {
    runBlocking {
        val produced = Baker.bake(12).map {
            ...
            val inspected = async { Inspector.inspect(boxed.await()) }
            inspected
        }
...
object Inspector {
    ...
    suspend fun inspect(box: Box): Box = withContext(dispatcher) {
        println("🔍 Inspecting product")
        when {
            rng.nextInt(100) < 20 -> {
                println("⛔ Decoration failed: Cupcake #${box.cupcake.id}")
                inspect(Box(Decorator.decorate(box.cupcake)))
            }

            rng.nextInt(100) < 10 -> {
                println("⛔ Box failed: Cupcake #${box.cupcake.id}")
                inspect(Boxer.box(box.cupcake))
            }
