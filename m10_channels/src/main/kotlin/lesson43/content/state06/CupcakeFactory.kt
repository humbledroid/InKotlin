package lesson43.content.state06
...
class Inspector(
    ...
    suspend fun start() {
        while (currentCoroutineContext().isActive) {
            select {
                fromDecorator.onReceive(::onCupcakeReceived)
                fromBoxer.onReceive(::onBoxReceived)
            }
        }
    }

    suspend fun onCupcakeReceived(cupcake: Cupcake) {
    ...
    suspend fun onBoxReceived(box: Box) {
        println("🔍 Inspecting product")
