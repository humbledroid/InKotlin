package lesson42.content.state03
...
fun main() = runBlocking(Dispatchers.Default) {
    ...
    val decoratedChannel = Channel<Cupcake>(capacity = BUFFERED)
    val boxChannel = Channel<Box>(capacity = BUFFERED)

    launch { Baker(countChannel, bakedChannel).start() }
    repeat(6) { launch { Decorator(bakedChannel, decoratedChannel).start() } }
